...
export function searchActiveRoutine(routines: Array<Routine | any>) {
   ...
   routines.sort((a, b) => {
      ...
      else return a.endTime - b.startTime;
   });
   ...
   if (routines.length > 0 && routines[0].status === 'done') {
      ...
      const doneRoutines = [];
      for (let i = 0; i < routines.length; i++) if (routines[i].status === 'done') doneRoutines.push(routines[i]);
...
function dayFilterMonthly(routine: Routine, date: Date) {
   ...
   if (currentDate in routine.time) {
      ...
      if (startTimeRoutine) routine.startTime = getNewDateSetHM(date, startTimeRoutine);
      if (endTimeRoutine) routine.endTime = getNewDateSetHM(date, endTimeRoutine);
      return true;
   }
   return false;
}
...
function dayFilterOnce(routine: Routine, now: Date) {
   ...
   if (isSameDate(startTime, now)) {
      ...
      if (routine.time[1]) routine.endTime = new Date(routine.time[1]);
      return true;
   }
   return false;
}
