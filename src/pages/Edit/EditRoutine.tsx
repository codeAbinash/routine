...
export default function EditRoutine() {
   ...
   return (
      <div className='screen min-h-full text-dark dark:text-darkText'>
         <BackHeader title='Edit Routine' backCb={handleBackClick} />
         <BottomModal show={modalShow} btnTxt={modalBtnText} cb={modalCallback}>
            {modalUi}
         </BottomModal>
         <div className='flex min-h-[calc(100vh-80px)] w-full flex-col items-center justify-between p-4'>
            ...
            <div className='btn w-full'>
               <button className='btn-full no-highlight tap99 w-full text-sm' onClick={saveRoutine}>
                  Save Routine
               </button>
            </div>
         </div>
   ...
   function handleBackClick() {
      setModalUi(<BasicModal text='Cancel Edit Routine?' desc='You will lose all the changes you made.' emoji='🧐' />);
   ...
   function saveRoutine() {
      ...
      if (!routineName)
         return defaultModal(
            <BasicModal text='Routine name is required' desc='Please provide a name for your routine' emoji='😕' />,
         );

      if (routineType === 'once') {
         if (!routine.time[0])
            return defaultModal(
               <BasicModal text='Routine time is required' desc='Please provide a time for your routine' emoji='😕' />,
            );
         ...
         if (status.text != 'success') return defaultModal(<BasicModal text={status.text} desc={status.desc} />);
         else routine.time = status.data;
      } else if (routineType === 'holiday' || routineType === 'calendar') {
         let routineTime = routine.time[0];
         if (!routineTime)
            return defaultModal(
               <BasicModal text='Please Select a date' desc='You have to select a date to continue' />,
            );
...
function saveRoutineToLs(
   routine: Routine,
   allRoutines: Routine[],
   allExpiredRoutines: Routine[],
   index: number,
